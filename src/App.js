...
const ROLES_INFO = {
  ...
  reporter:  { name: "기자",        emoji: "📰", color: "#f1c40f", bg: "#1a1400", border: "#5c4a00", team: "citizen", desc: "매 밤 한 명의 직업을 공개할 수 있습니다." },
...
// ── 공통 컴포넌트 ──
const Tag = ({ children, color = "#333", bg = "#111" }) => (
  <span style={{ background: bg, border: `1px solid ${color}`, borderRadius: 20, padding: "3px 10px", fontSize: 11, color, display: "inline-block" }}>
    {children}
  </span>
);
...
const Btn = ({ children, onClick, color = "#8B0000", disabled, style = {} }) => (
  <button onClick={onClick} disabled={disabled} style={{
    width: "100%", padding: "15px", background: disabled ? "#1a1a1a" : `linear-gradient(135deg, ${color}dd, ${color})`,
    color: disabled ? "#444" : "#fff", border: disabled ? "1px solid #222" : "none",
    borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer",
    fontFamily: "'Noto Sans KR', sans-serif", marginBottom: 10,
    boxShadow: disabled ? "none" : `0 4px 20px ${color}44`, transition: "all 0.2s", ...style,
  }}>{children}</button>
);

const Card = ({ children, style = {} }) => (
  <div style={{ background: "#111", borderRadius: 18, padding: "18px 20px", marginBottom: 14, width: "100%", maxWidth: 420, boxSizing: "border-box", ...style }}>
    {children}
  </div>
);

const PageWrap = ({ children, night = false }) => (
  <div style={{
    minHeight: "100vh",
    background: night
      ? "linear-gradient(180deg, #05050f 0%, #0a0a1a 100%)"
      : "linear-gradient(180deg, #0a0000 0%, #0d0d0d 100%)",
    color: "#fff", fontFamily: "'Noto Sans KR', sans-serif",
    display: "flex", flexDirection: "column", alignItems: "center",
    padding: "32px 20px 40px", transition: "background 1s ease",
  }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: "radial-gradient(circle, #8B0000 0%, #5a0000 70%, transparent 100%)",
        marginBottom: 24, opacity: 0.8,
        boxShadow: "0 0 60px #8B000066",
      }} />

      <p style={{ color: "#8B0000", fontSize: 13, letterSpacing: 5, marginBottom: 6, fontWeight: 700 }}>육은영반</p>
      <h1 style={{
        fontSize: 38, fontWeight: 900, color: "#fff", margin: "0 0 6px",
        textShadow: "0 0 40px #8B0000, 0 2px 4px rgba(0,0,0,0.8)",
        letterSpacing: 2, textAlign: "center",
      }}>마피아 게임</h1>
      <p style={{ color: "#5a0000", fontSize: 13, letterSpacing: 4, marginBottom: 48 }}>누가 마피아인가?</p>

      <div style={{ width: "100%", maxWidth: 360 }}>
        <Btn onClick={onHost} color="#8B0000">🏠 방 만들기</Btn>
        <Btn onClick={onJoin} color="#1a3a6b">🚪 방 참가하기</Btn>
      </div>
    </div>
  );
}
...
function HostScreen({ onCreated }) {
  ...
  const isValid = name.trim() && citizenCount >= 0 && playerCount >= 4;
  ...
  const create = async () => {
    ...
    await set(ref(db, `rooms/${code}`), {
      ...
      players: { [hostId]: { name: name.trim(), isHost: true, alive: true } },
    });
  ...
  return (
    <PageWrap>
      <h2 style={{ fontSize: 22, fontWeight: 900, marginBottom: 24, color: "#fff" }}>🏠 방 만들기</h2>

      <Card>
        <p style={{ color: "#666", fontSize: 11, marginBottom: 8 }}>닉네임</p>
        <input
          style={{ width: "100%", padding: "13px 16px", background: "#1a1a1a", color: "#fff", border: "1px solid #2a2a2a", borderRadius: 12, fontSize: 15, fontFamily: "'Noto Sans KR', sans-serif", boxSizing: "border-box", outline: "none" }}
          placeholder="이름을 입력하세요" value={name} onChange={e => setName(e.target.value)}
        />
      </Card>

      <Card>
        <p style={{ color: "#666", fontSize: 11, marginBottom: 12 }}>플레이어 수</p>
      ...
      <Card>
        ...
        {roleRows.map(r => (
          <div key={r.id} style={{ display: "flex", alignItems: "center", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 0, background: "#1a1a1a", borderRadius: 10, overflow: "hidden", border: "1px solid #2a2a2a" }}>
        ...
        <div style={{ borderTop: "1px solid #1a1a1a", paddingTop: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span style={{ fontWeight: 700, color: citizenCount < 0 ? "#e74c3c" : "#2ecc71", fontSize: 15 }}>
...
function JoinScreen({ onJoined }) {
  ...
  const join = () => {
    ...
    onValue(ref(db, `rooms/${code.toUpperCase()}`), async (snap) => {
      ...
      await update(ref(db, `rooms/${code.toUpperCase()}/players/${pid}`), { name: name.trim(), isHost: false, alive: true });
...
// ── 대기실 ──
function LobbyScreen({ code, playerId, onStart }) {
  const [room, setRoom] = useState(null);

  useEffect(() => onValue(ref(db, `rooms/${code}`), snap => setRoom(snap.val())), [code]);
  useEffect(() => { if (room?.status === "playing") onStart(); }, [room?.status]);
  ...
  return (
    <PageWrap>
      ...
      <div style={{ fontSize: 36, fontWeight: 900, letterSpacing: 10, color: "#fff", marginBottom: 4, textShadow: "0 0 20px #8B000088" }}>{code}</div>
      <p style={{ color: "#555", fontSize: 12, marginBottom: 28 }}>친구들에게 코드를 알려주세요</p>
      ...
      <Card>
        ...
        {players.map((p, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", padding: "11px 0", borderBottom: i < players.length - 1 ? "1px solid #1a1a1a" : "none" }}>
            <span style={{ marginRight: 10 }}>{p.isHost ? "👑" : "👤"}</span>
            <span style={{ flex: 1, fontSize: 14 }}>{p.name}</span>
            {p.isHost && <Tag color="#d4a017">방장</Tag>}
          </div>
        ))}
      </Card>

      {isHost
        ? <Btn onClick={() => update(ref(db, `rooms/${code}`), { status: "playing" })} color={isFull ? "#8B0000" : "#333"} disabled={!isFull}>
            {isFull ? "🎮 게임 시작!" : `${room.playerCount - players.length}명 더 필요해요`}
          </Btn>
        : <p style={{ color: "#555", fontSize: 13, textAlign: "center" }}>방장이 시작할 때까지 기다려주세요...</p>
      }
    </PageWrap>
  );
}
...
function RoleRevealScreen({ code, playerId, onReady }) {
  ...
  useEffect(() => {
    onValue(ref(db, `rooms/${code}`), snap => {
      ...
      if (idx >= 0 && room.roleList) setRole(room.roleList[idx]);
    }, { onlyOnce: true });
  }, []);
  ...
  const confirm = async () => {
    await update(ref(db, `rooms/${code}/players/${playerId}`), { role, ready: true });
    onReady(role);
  };
    ...
    <PageWrap>
      ...
      {!revealed ? (
        ...
        <div style={{
          ...
          boxShadow: `0 8px 40px ${ri.color}33`,
        }}>
          <span style={{ fontSize: 52 }}>{ri.emoji}</span>
          <h3 style={{ color: ri.color, fontSize: 22, fontWeight: 900, margin: "14px 0 8px", textShadow: `0 0 20px ${ri.color}` }}>{ri.name}</h3>
...
// ── 승리 화면 ──
function WinScreen({ winner, myRole, onRestart }) {
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: isMafiaWin ? "radial-gradient(ellipse at top, #2a0000, #000)" : "radial-gradient(ellipse at top, #002a00, #000)",
      display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
      fontFamily: "'Noto Sans KR', sans-serif", padding: 32, textAlign: "center",
    }}>
      ...
      <p style={{ fontSize: 18, color: iWon ? "#ffd700" : "#666", marginBottom: 48 }}>
        {iWon ? "🏆 당신이 이겼어요!" : "😢 아쉽게 졌어요..."}
      </p>
      <div style={{ width: "100%", maxWidth: 360 }}>
...
// ── 메인 게임 ──
function GameScreen({ code, playerId, myRole, onWin }) {
  const [room, setRoom] = useState(null);
  ...
  useEffect(() => {
    setNightTarget(null); setNightSubmitted(false);
  ...
  const isHost = playersMap[playerId]?.isHost;
  ...
  const needsNightAction = ["mafia", "mafiaBoss", "doctor", "police", "reporter"].includes(myRole) && !(myRole === "reporter" && reporterAlreadyUsed);
  ...
  const submitNightAction = async (targetId) => {
    ...
    let actionKey = null;
    if (myRole === "mafia" || myRole === "mafiaBoss") actionKey = "mafia";
    else if (myRole === "doctor") actionKey = "doctor";
    else if (myRole === "police") actionKey = "police";
    else if (myRole === "reporter") actionKey = "reporter";
    if (!actionKey) return;
    await update(ref(db, `rooms/${code}/nightActions`), { [actionKey]: targetId });
    if (myRole === "police") {
      const targetRole = playersMap[targetId]?.role;
      // 마피아 보스는 경찰 조사에 시민으로 나옴
      const actualResult = targetRole === "mafiaBoss" ? "시민" : isMafia(targetRole) ? "마피아" : "시민";
      setPoliceResult({ name: playersMap[targetId]?.name, result: actualResult });
    }
    setNightTarget(targetId); setNightSubmitted(true);
  };

  // ── 의사: 보호 제출 ──
  const submitDoctor = async (targetId) => {
    if (nightSubmitted || !amAlive) return;
    await update(ref(db, `rooms/${code}/nightActions`), { doctor: targetId });
    setNightTarget(targetId); setNightSubmitted(true);
  };
  ...
  const processNight = async () => {
    ...
    const mafiaTarget = nightActions.mafia;
    ...
    if (killed) updatedPlayers[killed] = { ...updatedPlayers[killed], alive: false, role: playersMap[killed]?.role };
    const win = checkWin(updatedPlayers);
    if (win) updates[`rooms/${code}/winner`] = win;
  ...
  const processVote = async () => {
    ...
    if (room.lawyerBlock) {
      await update(ref(db, `rooms/${code}`), {
        phase: "night", votes: null, lawyerBlock: null,
        lastExecution: { round, playerId: null, blocked: true },
      });
      return;
    }
    ...
    if (executed) {
      ...
      updates[`rooms/${code}/players/${executed}/deathRound`] = round;
    ...
    const updatedPlayers = { ...playersMap };
    if (executed) updatedPlayers[executed] = { ...updatedPlayers[executed], alive: false, role: playersMap[executed]?.role };
    const win = checkWin(updatedPlayers);
    if (win) updates[`rooms/${code}/winner`] = win;
    ...
    <PageWrap night={isNight}>
      ...
      {phase === "day" && room.reporterReveal?.round === round - 1 && (
        <Card style={{ border: "1px solid #5c4a00", background: "#1a1400" }}>
          ...
          <p style={{ fontSize: 15 }}>
            <strong>{room.reporterReveal.targetName}</strong>님의 직업은{" "}
            <span style={{ color: room.reporterReveal.result === "마피아" ? "#e74c3c" : "#2ecc71", fontWeight: 900, fontSize: 17 }}>
              {room.reporterReveal.result}
            </span>
            입니다!
          </p>
        </Card>
      ...
      {/* 처형 공지 */}
      {phase === "night" && lastExecution?.round === round - 1 && (
      ...
      {myRole === "police" && policeResult && (
        <Card style={{ border: "1px solid #1a3a5c", background: "#00091a" }}>
          ...
          <p style={{ fontSize: 15 }}>
            <strong>{policeResult.name}</strong>님은{" "}
            <span style={{ color: policeResult.result === "마피아" ? "#e74c3c" : "#2ecc71", fontWeight: 900, fontSize: 17 }}>
              {policeResult.result}
            </span>
            입니다!
          </p>
      ...
      {myRole === "lawyer" && phase === "vote" && amAlive && !room.lawyerUsed && (
        <Card style={{ border: "1px solid #3a1a5c", background: "#0d0018" }}>
          <p style={{ color: "#9b59b6", fontSize: 12, fontWeight: 700, marginBottom: 8 }}>⚖️ 변호사 능력 (1회 사용 가능)</p>
          <p style={{ color: "#888", fontSize: 12, marginBottom: 12 }}>이의를 제기하면 이번 처형이 취소됩니다!</p>
          <Btn onClick={useLawyer} color="#9b59b6" disabled={lawyerUsed || room.lawyerUsed} style={{ marginBottom: 0 }}>
            {lawyerUsed || room.lawyerUsed ? "✅ 이의 제기 완료" : "⚖️ 이의 있습니다!"}
          </Btn>
        </Card>
      )}
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          {phase === "night" && needsNightAction && amAlive && <p style={{ color: "#3498db", fontSize: 12 }}>탭해서 선택</p>}
        </div>
          ...
          {alivePlayers.map(([id, p]) => {
            ...
            return (
              <div key={id}
                onClick={() => {
                  if (!canClick) return;
                  if (phase === "vote") submitVote(id);
                  if (phase === "night") submitNightAction(id);
                }}
                style={{
                  display: "flex", alignItems: "center",
                  padding: "12px 14px", borderRadius: 12,
                  background: myVote || isNightSelected ? "#1a0a00" : isMe ? "#0d0d1a" : "#1a1a1a",
                  border: `1px solid ${myVote || isNightSelected ? "#5a3000" : isMe ? "#2a2a4a" : "#222"}`,
                  cursor: canClick ? "pointer" : "default",
                  transition: "all 0.15s",
                }}>
                <span style={{ marginRight: 10, fontSize: 16 }}>{p.isHost ? "👑" : "👤"}</span>
                <span style={{ flex: 1, fontSize: 14, fontWeight: isMe ? 700 : 400 }}>
                  {p.name}{isMe ? " (나)" : ""}
                </span>
                {myVote && phase === "vote" && (
                  <span style={{ background: "#3a0000", color: "#e74c3c", borderRadius: 20, padding: "2px 10px", fontSize: 12, fontWeight: 700 }}>
                    ✓ 투표함
                  </span>
                )}
                {isNightSelected && (
                  <span style={{ background: "#5a3000", color: "#f1c40f", borderRadius: 20, padding: "2px 10px", fontSize: 12 }}>선택됨</span>
                )}
              </div>
            );
          })}
      ...
      <Card style={{ borderLeft: `3px solid ${isNight ? "#3498db" : phase === "vote" ? "#e74c3c" : "#f1c40f"}`, background: "#0d0d0d" }}>
        {phase === "day" && <p style={{ color: "#888", fontSize: 13, lineHeight: 1.8 }}>☀️ 자유롭게 토론하세요. 방장이 투표를 시작하면 참여하세요.</p>}
        {phase === "night" && !amAlive && <p style={{ color: "#444", fontSize: 13 }}>💀 사망 상태입니다. 조용히 지켜보세요.</p>}
        {phase === "night" && amAlive && !needsNightAction && myRole !== "reporter" && <p style={{ color: "#666", fontSize: 13 }}>👤 눈을 감고 기다리세요...</p>}
        {phase === "night" && amAlive && myRole === "reporter" && reporterAlreadyUsed && <p style={{ color: "#5c4a00", fontSize: 13 }}>📰 이미 능력을 사용했습니다. 눈을 감고 기다리세요.</p>}
        {phase === "night" && amAlive && needsNightAction && !nightSubmitted && (
          <p style={{ color: "#3498db", fontSize: 13 }}>
            {(myRole === "mafia" || myRole === "mafiaBoss") && "🔫 위에서 제거할 대상을 선택하세요."}
            {myRole === "doctor" && "⚕️ 위에서 보호할 사람을 선택하세요."}
            {myRole === "police" && "🚔 위에서 조사할 사람을 선택하세요."}
            {myRole === "reporter" && "📰 위에서 공개할 사람을 선택하세요."}
          </p>
        )}
        {phase === "night" && amAlive && needsNightAction && nightSubmitted && (
          <p style={{ color: "#2ecc71", fontSize: 13 }}>✅ 행동 완료! 방장이 넘길 때까지 기다리세요.</p>
        )}
        {phase === "vote" && !amAlive && <p style={{ color: "#444", fontSize: 13 }}>💀 사망 상태라 투표할 수 없습니다.</p>}
        {phase === "vote" && amAlive && !voteSubmitted && <p style={{ color: "#e74c3c", fontSize: 13 }}>🗳️ 위에서 처형할 사람을 선택하세요!</p>}
        {phase === "vote" && amAlive && voteSubmitted && <p style={{ color: "#2ecc71", fontSize: 13 }}>✅ 투표 완료! ({playersMap[voteTarget]?.name}에게 투표)</p>}
      </Card>

      {/* 방장 컨트롤 */}
      {isHost && (
        <Card style={{ border: "1px solid #1a1a1a", background: "#0a0a0a" }}>
          <p style={{ color: "#444", fontSize: 11, marginBottom: 12 }}>👑 방장 컨트롤</p>
          {phase === "day" && (
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => update(ref(db, `rooms/${code}`), { phase: "vote" })}
                style={{ flex: 1, padding: "13px", background: "#1a0000", color: "#e74c3c", border: "1px solid #3a0000", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Noto Sans KR', sans-serif" }}>
                🗳️ 투표 시작
              </button>
              <button onClick={() => update(ref(db, `rooms/${code}`), { phase: "night" })}
                style={{ flex: 1, padding: "13px", background: "#00091a", color: "#3498db", border: "1px solid #1a3a5c", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Noto Sans KR', sans-serif" }}>
                🌙 밤으로
              </button>
            </div>
          )}
          {phase === "night" && (
            <button onClick={processNight}
              style={{ width: "100%", padding: "13px", background: "#001a00", color: "#2ecc71", border: "1px solid #1a5c2a", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Noto Sans KR', sans-serif" }}>
              ☀️ 밤 종료 → 결과 공개
            </button>
          )}
          {phase === "vote" && (
            <button onClick={processVote}
              style={{ width: "100%", padding: "13px", background: "#1a0000", color: "#e74c3c", border: "1px solid #3a0000", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Noto Sans KR', sans-serif" }}>
              ⚖️ 투표 종료 → {room.lawyerBlock ? "처형 취소 (변호사)" : "처형"}
            </button>
          )}
        </Card>
      )}
...
export default function App() {
  ...
  const reset = () => { setScreen("title"); setMyRole(null); setWinner(null); setRoomCode(""); setPlayerId(""); };

  return (
    <>
      <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700;900&display=swap" rel="stylesheet" />
      {screen === "title" && <TitleScreen onHost={() => setScreen("host")} onJoin={() => setScreen("join")} />}
      {screen === "host" && <HostScreen onCreated={(code, id) => { setRoomCode(code); setPlayerId(id); setScreen("lobby"); }} />}
      {screen === "join" && <JoinScreen onJoined={(code, id) => { setRoomCode(code); setPlayerId(id); setScreen("lobby"); }} />}
      {screen === "lobby" && <LobbyScreen code={roomCode} playerId={playerId} onStart={() => setScreen("role")} />}
      {screen === "role" && <RoleRevealScreen code={roomCode} playerId={playerId} onReady={(role) => { setMyRole(role); setScreen("game"); }} />}
      {screen === "game" && !winner && <GameScreen code={roomCode} playerId={playerId} myRole={myRole} onWin={setWinner} />}
      {screen === "game" && winner && <WinScreen winner={winner} myRole={myRole} onRestart={reset} />}
    </>
  );
}